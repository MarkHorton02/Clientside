...
const Signup = () => {
    ...
    const registerHandler = () => {
        axios.post("https://backend-final-copy-production.up.railway.app/users/register",user).then((res)=>{
                ...
                <div  className='signup-right'>
                    <div className='signup-right-gradient'>
                        <div className='register-container'>
                            ...
                            {username?.length<4 || fullname?.length<6 || email?.length<6 || password?.length<8 || password !==confirmPassword ? null : <button onClick={registerHandler} class="register-btn">Signup</button> }
                        </div>
                    </div>
                </div>
            </div>
